...
def create_train_data():
    train_data_path = os.path.join(data_path, 'trains')
...
def create_test_data():
    train_data_path = os.path.join(data_path, 'tests')
    images = os.listdir(train_data_path)
    total = int(len(images) / 2)
    ...
    for image_name in images:
        if 'mask' in image_name:
            continue
        img_mask_name = image_name.split('.')[0] + '_mask.png'
        img_id = int(image_name.split('.')[0])
        img = cv2.imread(os.path.join(train_data_path, image_name), cv2.IMREAD_GRAYSCALE)
        img_mask = cv2.imread(os.path.join(train_data_path, img_mask_name), cv2.IMREAD_GRAYSCALE)
